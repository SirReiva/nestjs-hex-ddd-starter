...
import { IAuthUserRepository } from '../domain/repository/auth-user-repository.interface';
import { AuthUserRedis } from './types/auth-user-redis.type';
...
export class AuthUserRedisRepository implements IAuthUserRepository {
	...
	toDomain(persistentUser: AuthUserRedis): AuthUserModel {
		const { id, role } = persistentUser;
		return new AuthUserModel(new VOUuid(id), new VOUserRole(role));
	}
	...
	async save(user: AuthUserModel): Promise<void> {
		...
		await this.redisClient.set(redisUser.id, redisUser.role);
	}
	 ...
	 */
	async findOne(userId: VOUuid): Promise<AuthUserModel> {
		const role: UserRoles = (await this.redisClient.get(
			userId.value
		)) as UserRoles;

		const persistentUser = { id: userId.value, role };
