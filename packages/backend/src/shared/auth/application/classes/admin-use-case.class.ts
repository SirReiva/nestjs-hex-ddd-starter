...
/**
 * Abstract class for role-based admin-only use cases
 */
export abstract class AdminUseCase implements IUseCase {
	/**
	...
	/**
	 * Check if author is an admin and throws unauthorized/forbidden exception if not.
	...
	async isAuthorAllowed(authorId: VOUuid): Promise<void> {
		...
		if (authorRole.role !== UserRoles.ADMIN) throw new ForbiddenRoleException();
	}
}
